...
## 📊 Dataset
The project uses the [Chest X-Ray Images (Pneumonia)](https://www.kaggle.com/paultimothymooney/chest-xray-pneumonia) dataset from Kaggle. It consists of validated pediatric chest X-ray images categorized into two classes: **Normal** and **Pneumonia**.
...
![X-ray Samples](images/xray_samples.png)
...
2. **Model Loading**: Load the trained weights using PyTorch:
   ```python
   model.load_state_dict(torch.load('models/pneumonia_resnet18.pth'))
   ```
3. **Run Inference**: Use the provided notebook in the `notebooks/` folder to run predictions on your own X-ray images.
